...
/**
 * ⚠️ 化祿不等於「好事」。
 * 祿的本義是「資源往這裡流、這一塊被啟動」——可能是進帳，也可能是支出。
 * 化祿入疾厄或父母，很常見的實際情況是醫藥費、照顧長輩、把錢和時間大量
 * 投進去。之前把祿寫成「順風、會順起來」，導致「長輩住院」被講成「長輩順」。
 */
export const MUTAGEN = {
  祿: { name: '被啟動', verb: '會變得活絡',
    desc: '這一塊會被啟動，事情變多、資源往這裡流。多半是好的，但也可能是「必須把錢和時間投進去」' },
  權: { name: '掌控', verb: '你會拿到更多主導權',
    desc: '你在這裡說話會更有份量、能做的決定更多——但責任和壓力也會一起變大' },
  科: { name: '名聲', verb: '會被看見',
    desc: '這裡會有貴人、有好名聲，就算遇到麻煩也容易有人出手幫你化解' },
  忌: { name: '卡關', verb: '會卡住',
    desc: '這是最需要留神的地方，容易糾結、耗損、或是事情不如預期。不是不能碰，是要多花心思、別硬來' },
};

/** 化祿落在這些宮位時，「被啟動」通常伴隨支出或勞心，要講清楚 */
export const LU_COSTS = {
  疾厄: '但這一塊被啟動，常常是健康花錢、看醫生、或身體需要你投入照顧',
  父母: '但這一塊被啟動，常常是長輩的事變多——照顧、醫藥費、或要花時間陪',
  子女: '但這一塊被啟動，也可能是為了小孩或作品大量投入時間和錢',
  兄弟: '但這一塊被啟動，也可能是平輩來找你幫忙、或有金錢往來',
  僕役: '但這一塊被啟動，也可能是應酬變多、或有人來找你合作出資',
};
